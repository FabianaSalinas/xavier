...
export default function Rastreio() {
  ...
  const [showResult, setShowResult] = useState(false); // controla fade / exibição
  const resultRef = useRef(null);

  // ✅ opcional (se não tiver, ele some sozinho)
  ...
  const buscar = async (e) => {
    ...
    try {
      ...
      if (!first) {
        ...
        return; // ✅ não rola, não diminui
      }

      // ✅ achou: reduz hero + mostra resultado com fade + rola
      setDoc(first);
      setShowResult(true);

      // deixa o React renderizar antes de rolar
      setTimeout(() => {
  ...
  const heroCollapsed = Boolean(doc); // ✅ quando tem doc, colapsa o HERO
    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <section
        className={[
          ...
          "transition-[min-height,padding] duration-700 ease-in-out",
          heroCollapsed ? "min-h-[520px]" : "min-h-screen",
        ].join(" ")}
      >
        ...
        <div
          className={[
            ...
            heroCollapsed ? "py-10 sm:py-12" : "py-14 sm:py-16",
          ].join(" ")}
        >
      ...
      <section ref={resultRef} className="max-w-6xl mx-auto px-4 py-10">
        ...
        >
          {doc && (
            <div className="bg-white rounded-3xl shadow-sm ring-1 ring-slate-200 overflow-hidden">
              ...
              {/* Tracking ocupa toda a parte de baixo */}
              {doc.LinkTracking && (
        ...
        {!doc && (
          <div className="text-center text-slate-500">
            <p className="text-sm">Preencha os dados acima para exibir o status do documento.</p>
          </div>
        )}
      </section>
    </main>
